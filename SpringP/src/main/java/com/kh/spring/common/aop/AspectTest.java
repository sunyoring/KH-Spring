...
public class AspectTest {
	...
	//매개변수 JoinPoint 가 젤 앞에 와야함
	@Before("beforePointCut()")  //또는 "execution(* com.kh.spring..*ServiceImpl.*(..))" 를 직접 적어줘도 됨.
	...
	//매개변수 JoinPoint 가 젤 앞에 와야함   //에러 ,예외 상관없이 끝나면 무조건 수행  (finally 비슷)
	@After("afterPointCut()")  //또는 "execution(* com.kh.spring..*ServiceImpl.*(..))" 를 직접 적어줘도 됨.
	...
	//매개변수 JoinPoint 가 젤 앞에 와야함  //정상적으로 수행되었을때 결과를 리턴해줌 
	@AfterReturning(pointcut="afterReturning()", returning = "returnObj")  //반환값 명시해줘야함.
	...
	@Around("aroundPointCut()")   //조인포인트를 기준으로  앞 뒤 로 수행한다.
	public Object aroundPointCut(ProceedingJoinPoint join)throws Throwable { 
		...
		Object obj = join.proceed(); //전 후를 나누는 기준
